...
\begin{itemize}
    ...
    \item dependencias instaladas con \texttt{pip install -r requirements.txt};
    \item opcionalmente Docker y Docker Compose;
    \item Ollama instalado si se desea respuesta generada localmente.
\end{itemize}
...
python3 main.py web_search "playas cubanas" --top-k 5
\end{verbatim}

También puede ejecutarse la interfaz visual con:

\begin{verbatim}
python3 src/frontend/app.py
\end{verbatim}

Si se usa Docker, el sistema puede correr en un entorno reproducible sin depender de la configuración local.
...
\begin{itemize}
    \item \textbf{Cobertura del corpus limitada}: el sistema depende mucho de las páginas iniciales y de la calidad del crawling. Solución: ampliar seeds, automatizar nuevas reglas por sitio y programar re-crawls periódicos.
    \item \textbf{Heurísticas frágiles}: algunos filtros de calidad pueden rechazar contenido válido. Solución: entrenar clasificadores ligeros de calidad documental o ajustar umbrales por dominio.
    \item \textbf{Evaluación todavía parcial}: falta una batería más amplia de consultas y juicios de relevancia. Solución: construir un conjunto de evaluación más grande y medir precisión, cobertura y latencia por módulo.
    \item \textbf{Interfaz funcional pero sobria}: la UI cumple su rol, pero puede enriquecerse. Solución: agregar resaltado de citas, filtros por fuente, historial y vista expandida de resultados.
    \item \textbf{Búsqueda web costosa en casos extremos}: aunque se limita por número de páginas, puede seguir siendo lenta en sitios complejos. Solución: incorporar colas asíncronas, cache de respuestas y priorización más agresiva.
    \item \textbf{Dependencia de Ollama para la generación}: si el modelo local no está disponible, la respuesta se degrada. Solución: ofrecer más de un backend generativo o una respuesta extractiva de respaldo mejorada.
\end{itemize}
